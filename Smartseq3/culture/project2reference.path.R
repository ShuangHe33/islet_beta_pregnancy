setwd('G:/project/pregnant_mouse/beta/sm3/ref/new_sm3/ref_final/KO/project/20220913')
setwd('direct/20221120/')
...
# seu.A485Nif <- readRDS('G:/project/pregnant_mouse/beta/sm3/ref/new_sm3/20230606/pre/seu.ref.raw.islet.celltype.20230606.rds')
# seu.A485Nif <- subset(seu.A485Nif,cells = colnames(seu.A485Nif)[seu.A485Nif$CellType=='beta cell'])

# seu.vitro.nor.list[['sm3.Nif.A485']]@assays$RNA@data <- Matrix::Matrix(log(MyNorm(as.matrix(MyCalTp0.1mExcludeGene(as.matrix(seu.vitro.nor.list[['sm3.Nif.A485']]@assays$RNA@counts),
#                                                                                                                    genes.inf.input[rownames(seu.vitro.nor.list[['sm3.Nif.A485']]@assays$RNA@counts),'GeneLength'],
#                                                                                                                    c('Ins1','Ins2'))))+1),sparse = T)
...
#seu.vivo.vitro.list <- list()
# seu.vitro.nor.list$sm3.Nif.add$KOtype <- as.character(seu.vitro.nor.list$sm3.Nif.add$Type)
# seu.vivo.vitro.list[['sm3.Nif.add']] <- merge(seu.ges.beta,seu.vitro.nor.list$sm3.Nif.add)
# seu.vivo.vitro.list[['sm3.Nif.add']]@meta.data[seu.vivo.vitro.list[['sm3.Nif.add']]$KOtype=='/','Rep'] <- 'rep1'
# seu.vivo.vitro.list[['sm3.Nif.add']]$KOtype <- factor(seu.vivo.vitro.list[['sm3.Nif.add']]$KOtype,levels = c('/',names(table(seu.vitro.nor.list$sm3.Nif.add$Type))))
...
pdf(paste('G:/lab/Article/heshuang/BYLW/sm3/culture/',seu,'.rep.project.norep.pdf',sep = ''),width,height)
  # seu.vivo.vitro.list[[seu]]$KOtype2 <- seu.vivo.vitro.list[[seu]]$KOtype
  # seu.vivo.vitro.list[[seu]]$KOtype2 <- factor(seu.vivo.vitro.list[[seu]]$KOtype2,levels = names(table(seu.vivo.vitro.list[[seu]]$KOtype))[names(table(seu.vivo.vitro.list[[seu]]$KOtype))!='/'])
  #   seu.vivo.vitro.list[[seu]] <- ScaleData(seu.vivo.vitro.list[[seu]],features = ref.var.co.gene)
  #   seu.vivo.vitro.list[[seu]] <- RunPCA(seu.vivo.vitro.list[[seu]],features = ref.var.co.gene,npcs = 5)
  #   pca.project.ko.list[[seu]] <- scale(t(seu.vivo.vitro.list[[seu]]@assays$RNA@data[rownames(seu.ges.beta@reductions$pca@feature.loadings),]))%*% seu.ges.beta@reductions$pca@feature.loadings
  #   seu.vivo.vitro.list[[seu]]@reductions$pca@cell.embeddings <- pca.project.ko.list[[seu]][seu.vivo.vitro.list[[seu]]$SampleName,]
  # 
  ...
  # plot(p.pca+
  #        scale_color_manual(values = ko.time.col) +
...
# library(ggord)
# library(ggplot2)
# for(seu in names(seu.vivo.vitro.list)){
#   pca.res <- FactoMineR::PCA(t(as.matrix(seu.vivo.vitro.list[[seu]]@assays$RNA@data[ref.var.co.gene,])))
#   pca.res$ind$coord[,1] <- seu.vivo.vitro.list[[seu]]@reductions$pca@cell.embeddings[,'PC_1']
#   pca.res$ind$coord[,2] <- seu.vivo.vitro.list[[seu]]@reductions$pca@cell.embeddings[,'PC_2']
#   pdf(paste('direct/20221120/',seu,'circle.0.5.pdf'),15,12)
#   print(ggord(pca.res,seu.vivo.vitro.list[[seu]]$KOtype,col = c('gray80',time.colors[1:4]),arrow=NULL,vec_ext =0,txt=NULL,ellipse=T,ellipse_pro=0.5,
#         poly = F,alpha=0.5,size=I(5))+
#     theme_bw() + 
...
# for(seu in names(seu.vivo.vitro.list)){
#   print(seu)
#   si.select <- seu.vivo.vitro.list[[seu]]@meta.data
#   pregScore=c()
#   for (time in names(table(seu.vivo.vitro.list[[seu]]$Type))) {
#     regress=lm(-Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,2]~
#                  Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,1])$coefficients[2]
#     temp=-c(1,regress)%*%rbind(Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,1],
#                                Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,2])
#     # temp=temp-mean(temp)
#     names(temp)=si.select[si.select$Type==time,]$SampleName
#     pregScore=c(pregScore,temp)
#   }
#   seu.vivo.vitro.list[[seu]]@meta.data[names(pregScore),'pregScore'] <- pregScore
#   seu.vivo.vitro.list[[seu]]$pseudotime <- seu.vivo.vitro.list[[seu]]$pregScore
#   #seu.vivo.vitro.list[[seu]]$pseudotime <- -Embeddings(seu.vivo.vitro.list[[seu]],'pca')[,1]
# }




# for(seu in names(seu.vivo.vitro.list)[7]){
#   print(seu)
#   si.select <- seu.vivo.vitro.list[[seu]]@meta.data
#   pregScore=c()
#   for (time in names(table(seu.vivo.vitro.list[[seu]]$Type))) {
#     regress=lm(-Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,3]~
#                  Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,2])$coefficients[2]
#     temp=-c(1,regress)%*%rbind(Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,2],
#                                Embeddings(seu.vivo.vitro.list[[seu]],'pca')[si.select$Type==time,3])
#     # temp=temp-mean(temp)
#     names(temp)=si.select[si.select$Type==time,]$SampleName
#     pregScore=c(pregScore,temp)
#   }
...
#print(MyPseudotimebox(seu.vivo.vitro.list[[seu]]@meta.data,time.colors = time.colors)+theme(aspect.ratio = 1))
...
#     pdf(paste('direct/20221120/',seu,'ref.project.pseudobox.2.pdf',sep = ''),width,height)
#     #select.si.list[[seu]]$pseudotime <- -select.si.list[[seu]]$pseudotime
#     print(MyPseudotimebox(select.si.list[[seu]],time.colors = time.colors,size.point = 3,box.lwd = 1.5)+theme(aspect.ratio = 1.5))
#     dev.off()
# }
# #########pval#######
...
# 
# for(seu in names(select.si.list)){
#   #MyWriteTable(projct.pval.list[[seu]],row.names = T,paste('direct/20221120/',seu,'ref.project.pval.tab',sep = ''))
#   MyWriteTable(select.si.list[[seu]],row.names = T,paste(seu,'.ref.si.tab',sep = ''))
#   MyWriteTable(table(select.si.list[[seu]]$Type),row.names = T,paste(seu,'.ref.type.count.tab',sep = ''))
# }
########
ko.coord.list <- list()
pca.df.list <- list()
